...
public class MediaController extends FrameLayout {
	...
	private OnSeekBarChangeListener mSeekListener = new OnSeekBarChangeListener() {
		...
		public void onStartTrackingTouch(SeekBar bar) {
			if (!mInstantSeeking)
				mPlayer.seekTo((mDuration * 10) / 1000);
			if (mInfoView != null) {
				mInfoView.setText("");
	...
	private void initControllerView(View v) {
		...
		//快退、快进（上一集/下一集按钮）
		mPreviousButton = (ImageButton) v.findViewById(R.id.mediacontroller_previous);
		if (mPreviousButton != null) {
			mPreviousButton.requestFocus();
			mPreviousButton.setOnClickListener(mPreviousListener);
		}
		mNextButton = (ImageButton) v.findViewById(R.id.mediacontroller_next);
		if (mNextButton != null) {
